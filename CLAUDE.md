...
- **Inventory viewer** (player detail page): `GET /api/inventory/{name}` returns the
  ...
  custom names from JSON text components, damage — and EVERY other component
  rendered readably via `_component_lines` (trim, potion, lore, shulker contents,
  attribute modifiers, stored_enchantments on books; generic fallback for the rest).
  The tooltip never elides information; it centers over the hovered slot. Player
